...
class Valid
{
    ...
    {
        return preg_match("/^[0]?1(3[0-9]|4[57]|5[0-35-9]|8[0-9]|7[01678])\d{8}$/", $str);
    }
     ...
     * @return boolean
     */
    public static function max($str, $len)
    {
    ...
    public static function user_name($user_name)
    {
        return preg_match('/^[a-z0-9\x{4e00}-\x{9fa5}]+(_*[a-z0-9\x{4e00}-\x{9fa5}]+)*$/iu', $user_name)
            && strlen($user_name) >= 5 && mb_strlen($user_name, 'UTF-8') <= 30;
    }

    /**
     * 用户昵称 姓名昵称合法性检查，只能输入中文英文数字
     * @param $str string
     * @param $min_len int
     * @param $max_len int
     * @return boolean
     */
    public static function is_nickname($str, $min_len = 2, $max_len = 30)
    {
        if (preg_match("/^[\x80-\xffa-zA-Z0-9_-]{{$min_len},{$max_len}}$/", $str)) {
            return true;
        }
        return false;
    }
     ...
     * @return false|int
     */
    public static function usign($str)
    {
        return preg_match("/^([a-z0-9])+$/i", $str);
    }
}